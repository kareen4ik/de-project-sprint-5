...
class CourierLoader:
    ...
    WF_KEY = "example_courier_data_origin_to_stg_workflow"
    LAST_LOADED_TS_KEY = "last_loaded_ts"

    def __init__(self, collection_loader: CourierReader, pg_dest: PgConnect, pg_saver: CourierPgSaver, logger: Logger) -> None:
    ...
    def run_copy(self) -> int:
        with self.pg_dest.connection() as conn:
            ...
            load_queue = self.collection_loader.get_couriers(limit=self._SESSION_LIMIT)
            self.log.info(f"Found {len(load_queue)} couriers to sync.")
            if not load_queue:
            ...
            for courier in load_queue:
                try:
                    ...
                    if i % self._LOG_THRESHOLD == 0:
                        self.log.info(f"Processed {i} couriers out of {len(load_queue)}.")
                except Exception as e:
                    self.log.error(f"Failed to process courier: {courier}. Error: {e}")
